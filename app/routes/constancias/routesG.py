...
#-----------------------------------------PRINCIPAL-------------------------------------------------
@constancias.route("/constancias_grabaciones")
@login_required
def constancias_grabaciones():
    ...
    return render_template('constancias/constancias_grabaciones.html',
...
@constancias.route("/constancias_grabaciones/filtros") 
@login_required
def constancias_grabaciones_filtros():
...
#------------------------------------------------------DETALLES-----------------------------------------------------
@constancias.route("/constancias_grabaciones/detalles/<int:id>")
@login_required
def constancias_grabaciones_detalles(id):
     with get_db_connection() as con:
        ...
        if participantes is None:
            ...
            return redirect(url_for('constancias.constancias_grabaciones'))
        return render_template('constancias/constancias_grabaciones_detalles.html', 
                               participantes = participantes)
     
#----------------------------------------------------------GENERADOR DE CONSTANCIAS-------------------------------------------------------------
@constancias.route('/constancias_grabaciones/folio/', methods=["POST"])
@login_required
def folio_constancia():
    ...
    'constancias.constancias_grabaciones_generar',
    id=id,
    curso=curso,
...
@constancias.route("/constancias_grabaciones/folio/generar")
@login_required 
def constancias_grabaciones_generar():
    id = request.args.get("id")
    curso = request.args.get("curso")
    fecha_curso = request.args.get("fecha_curso")  # formato ISO: 'YYYY-MM-DD'
...
#--------------------------------------------------------------EDITAR PARTICIPANTE----------------------------------------------------------------------------
@constancias.route("/constancias_grabaciones/participantes")
@login_required
def constancias_grabaciones_editar():
    id = request.args.get("id")
    curso = request.args.get("curso")
    ...
    cur.close()
    con.close()
    return render_template('constancias/constancias_grabaciones_editar.html', 
                           participante = participante[0], 
                           cursos = lista_cursos(), 
                           categorias = lista_categorias(), 
                           ponentes = lista_ponente())

@constancias.route("/constancias_grabaciones/actualizar/<int:id>", methods = ['POST'])
@login_required
def constancias_grabaciones_actualizar(id):
    datos = request.form
    con = get_db_connection()
    cur = con.cursor()
...
#-----------------------------------------------------------------------------------------------------------------
@constancias.route('/constancias_grabaciones/modificar', methods=['POST'])
@login_required
def modificar_constancias_grabaciones():
